...
{
    FILE* fout = fopen(_file_name.c_str(), "w+");
    if (!fout)
        return false;

    if (_weighted)
        fprintf(fout, "WeightedAdjacencyGraph\n");
    else
        fprintf(fout, "AdjacencyGraph\n");
    
    fprintf(fout, "%d\n%lld\n", _vertices_count, _edges_count);

    // Converting from edges list to adjacency list
    vector<vector<int> > adjacencies(_vertices_count);
    vector<vector<int> > weights_adj(_vertices_count);

    if (_weighted)
    {
        for (long long i = 0; i < _edges_count; i++)
        {
            adjacencies[_src_ids[i]].push_back(_dst_ids[i]);
            weights_adj[_src_ids[i]].push_back(_weights[i]);
        }
    }
    else
    {
